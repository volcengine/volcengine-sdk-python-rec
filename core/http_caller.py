...
from core.constant import VOLC_AUTH_SERVICE

from core.context import Context
from core.exception import NetException, BizException
from core.option import Option
from core.options import Options
from core.time_helper import milliseconds
from volcauth.volcauth import VolcAuth
from core.metrics_helper import report_request_success, report_request_error, report_request_exception
from core.constant import METRICS_KEY_INVOKE_ERROR, METRICS_KEY_INVOKE_SUCCESS
...
class HttpCaller(object):
    ...
    def do_request(self, url, req_bytes, response, contextType, options: Options):
        ...
        if rsp_bytes is not None:
            try:
                ...
                log.error("[VolcengineSDK] parse response fail, err:%s url:%s", e, url)
    ...
    def _with_options_headers(headers: dict, options: Options):
        ...
        if options.request_id is not None and len(options.request_id) > 0:
            ...
            log.info("[VolcengineSDK] use requestId generated by sdk: '%s' ", request_id)
            headers["Request-Id"] = str(request_id)
        if options.data_date is not None:
            headers["Content-Date"] = options.data_date.strftime("%Y-%m-%d")
    ...
    def _do_http_request(self, url: str, headers: dict,
        ...
        bytes]:
        start = time.time()
        # log.debug("[VolcengineSDK][HTTPCaller] URL:%s Request Headers:\n%s", url, str(headers))
        ...
        try:
            ...
            if self._is_timeout_exception(e):
                log.error("[VolcengineSDK] do http request timeout, url:%s msg:%s", url, e)
                raise NetException(str(e))
            log.error("[VolcengineSDK] do http request occur io exception, url:%s msg:%s", url, e)
            raise BizException(str(e))
        finally:
            cost = int((time.time() - start) * 1000)
            log.debug("[VolcengineSDK] http path:%s, cost:%dms", url, cost)
        # log.debug("[VolcengineSDK][HTTPCaller] URL:%s Response Headers:\n%s", url, str(rsp.headers))
    ...
    def _log_rsp(url: str, rsp: Response) -> None:
        ...
        if rsp_bytes is not None and len(rsp.content) > 0:
            log.error("[VolcengineSDK] http status not 200, url:%s code:%d msg:%s headers:\n%s body:\n%s",
